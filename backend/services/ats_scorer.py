...
def _skill_matches(skill: str, text: str, embedder: SentenceTransformer, threshold: float) -> Tuple[bool, float]:
    # fast, O(n) direct check if skill is a substring of the text (case-insensitive)
...
# 3. content quality score (max 25)
...
# 4. skill validation score (max 15)
...
# 5. ATS compatibility score (max 15)
...
def calculate_overall_score(
    ...
    # The five components are already on a 20/25/25/15/15 scale that sums to
    # 100, so the overall score is a direct sum — no re-weighting. This
    # guarantees the displayed breakdown always adds up to the headline
    # score, matching what the synopsis documents.
    base_score = (
        formatting_score
        + keywords_score
        + content_score
    ...
    # Recorded for display only — grammar and location penalties are already
    # baked into content_score / ats_compatibility_score above, so they are
...
# Strengths based on component scores
